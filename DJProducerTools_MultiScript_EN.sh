...
ensure_base_path_valid() {
  ...
  if [ "${#state_candidates[@]}" -gt 0 ]; then
    echo "Detected states (_DJProducerTools):"
    idx=1
    for sc in "${state_candidates[@]}"; do
      echo "  [$idx] $sc"
      idx=$((idx + 1))
    done
    echo "  [S] Skip auto selection"
    printf "Pick a number to use as BASE_PATH: "
    read -r sel
...
init_paths() {
  # Prefer launch directory if it already contains _DJProducerTools
  if [ -n "$LAUNCH_PATH" ] && [ -d "$LAUNCH_PATH/_DJProducerTools" ]; then
    if [ "$BASE_PATH" != "$LAUNCH_PATH" ]; then
      printf "%s[INFO]%s BASE_PATH adjusted to launch directory: %s\n" "$C_CYN" "$C_RESET" "$LAUNCH_PATH"
    fi
  ...
  case "$BASE_PATH" in
    */_DJProducerTools|*/_DJProducerTools/)
      ...
      printf "%s[WARN]%s Adjusted BASE_PATH (should not point to _DJProducerTools): %s\n" "$C_YLW" "$C_RESET" "$BASE_PATH"
      ;;
  esac
...
maybe_reuse_file() {
  ...
  if [ -s "$file" ]; then
    ...
    printf "%s[INFO]%s %s already exists (%s, %s). Reuse (R) or regenerate (g)? [R/g]: " "$C_YLW" "$C_RESET" "$desc" "${meta_date:-n/a}" "${meta_size:-n/a}"
    read -r reuse
    case "$reuse" in
      g|G) return 0 ;;
      *) printf "%s[OK]%s Reusing %s\n" "$C_GRN" "$C_RESET" "$file"; pause_enter; return 1 ;;
    esac
  fi
  return 0
}
...
print_header() {
  ...
  if [ -f "$BANNER_FILE" ]; then
    printf "%s" "$C_PURP"
    # Clean literal "\n" endings that might be stored in banner file
...
action_1_status() {
  ...
  if [ -n "${EXTRA_SOURCE_ROOTS:-}" ]; then
    printf "  EXTRA_SOURCE_ROOTS (auto-detected on start): %s\n" "$EXTRA_SOURCE_ROOTS"
  fi
...
action_10_dupes_plan() {
  ...
  if [ -n "$meta" ]; then
    ...
    if [ "$meta_age" -gt 168 ]; then
      printf "%s[WARN]%s hash_index is older than 7 days (%s, %s, %sh). Regenerate before planning? (y/N): " "$C_YLW" "$C_RESET" "$meta_date" "$meta_size" "$meta_age"
      read -r regen
      case "$regen" in
  ...
  HASH_FILE="$hash_file" PLAN_TSV="$plan_tsv" run_with_spinner "DUPES" "Generating plan..." bash -c '
  BEGIN { FS=OFS="\t" }
  {
    if (NF < 3) next
    h=$1
...
action_11_quarantine_from_plan() {
  ...
  if [ -n "$meta" ]; then
    meta_date=$(echo "$meta" | cut -d'|' -f1)
    meta_age=$(echo "$meta" | cut -d'|' -f3)
    printf "%s[INFO]%s Current plan: %s (age: %sh)\n" "$C_CYN" "$C_RESET" "$meta_date" "$meta_age"
    if [ "$meta_age" -gt 168 ]; then
      printf "%s[WARN]%s Plan older than 7 days. Continue? (y/N): " "$C_YLW" "$C_RESET"
      read -r cont_old
      case "$cont_old" in
        y|Y) ;;
        *) printf "%s[INFO]%s Cancelled. Re-generate plan (option 10).\n" "$C_CYN" "$C_RESET"; pause_enter; return ;;
      esac
    fi
  fi
  ...
  if [ "$avail_bytes" -lt "$needed_bytes" ] && [ "$needed_bytes" -gt 0 ]; then
    printf "%s[WARN]%s Not enough space in quarantine.\n" "$C_YLW" "$C_RESET"
    printf "Use 'mark only' mode (no moves, just preview)? (y/N): "
    read -r mark_ans
    case "$mark_ans" in
      y|Y) mark_only=1 ;;
      *)
        printf "Continue moving anyway despite low space? (y/N): "
        read -r space_ans
        case "$space_ans" in
          y|Y) ;;
      ...
      3)
        if [ "$SAFE_MODE" -eq 1 ] || [ "$DJ_SAFE_LOCK" -eq 1 ]; then
          ...
          if [ "${q_count_del:-0}" -eq 0 ]; then
            printf "%s[INFO]%s Quarantine is empty.\n" "$C_CYN" "$C_RESET"
            pause_enter
            continue
          fi
          printf "%s[WARN]%s Delete ALL files in quarantine (only under %s).\n" "$C_YLW" "$C_RESET" "$QUAR_DIR"
          printf "Files: %s | Size: %s\n" "$q_count_del" "${q_size_del:-n/a}"
          printf "Preview (relative paths inside quarantine):\n"
          find "$QUAR_DIR" -type f 2>/dev/null | sed "s|$QUAR_DIR/||" | head -10
          printf "Confirm (YES to delete permanently): "
          read -r ans2
          if [ "$ans2" = "YES" ]; then
            rm -rf "$QUAR_DIR"/*
...
action_15_relink_helper() {
  ...
  if [ -s "$out" ]; then
    ...
    printf "%s[INFO]%s relink_helper.tsv already exists (%s, %s).\n" "$C_YLW" "$C_RESET" "${meta_date:-n/a}" "${meta_size:-n/a}"
    printf "Reuse (R) or regenerate (g)? [R/g]: "
    read -r reuse
    case "$reuse" in
      g|G) ;;
      *)
        printf "%s[OK]%s Reusing existing files:\n  %s\n  %s\n" "$C_GRN" "$C_RESET" "$out" "$doctor_out"
        pause_enter
        return
        ;;
    esac
  ...
  include_hash=0
  printf "Include SHA-256 hash per file? (may be slow) [y/N]: "
  read -r hash_ans
  case "$hash_ans" in
    y|Y) include_hash=1 ;;
    *) include_hash=0 ;;
  ...
  install_note="(no install attempted)"
  if [ "${#missing_tools[@]}" -gt 0 ]; then
    printf "%s[WARN]%s Missing tools: %s\n" "$C_YLW" "$C_RESET" "${missing_tools[*]}"
    printf "Attempt auto-install with brew/pip? (y/N): "
    read -r inst
    case "$inst" in
      y|Y)
        install_note="(install attempted)"
        if command -v brew >/dev/null 2>&1; then
          ...
          if [ "${#todo[@]}" -gt 0 ]; then
            brew install "${todo[@]}" || echo "[WARN] brew install failed" | tee -a "$doctor_out"
          fi
        else
          echo "[WARN] brew not available" | tee -a "$doctor_out"
        fi
        if command -v python3 >/dev/null 2>&1; then
          python3 -m pip install --user mutagen >/dev/null 2>&1 && echo "[OK] mutagen installed" | tee -a "$doctor_out" || echo "[WARN] mutagen install failed" | tee -a "$doctor_out"
        fi
        ;;
      *) ;;
    esac
  fi
  {
    ...
    echo
    if [ "${#missing_tools[@]}" -gt 0 ]; then
      echo "Missing tools $install_note:"
      printf "  - %s\n" "${missing_tools[@]}"
      echo
    ...
    if [ "$include_hash" -eq 1 ]; then
      echo "  - Use relink_helper.tsv (col1=relative, col2=absolute, col3=size, col4=mtime, col5=hash) to map/move files."
    else
...
action_27_snapshot() {
  ...
  if [ -f "$out" ]; then
    ...
    meta_size=$(echo "$meta" | cut -d'|' -f2)
    meta_age=$(echo "$meta" | cut -d'|' -f3)
    printf "%s[INFO]%s snapshot already exists (%s, %s, %sh).\n" "$C_YLW" "$C_RESET" "$meta_date" "$meta_size" "$meta_age"
    printf "Reuse (R) or regenerate (g)? [R/g]: "
    read -r reuse
    case "$reuse" in
      g|G) ;;
      *) printf "%s[OK]%s Reusing existing snapshot.\n" "$C_GRN" "$C_RESET"; pause_enter; return ;;
    esac
  fi
...
action_mirror_integrity_check() {
  ...
  for f in "$file_a" "$file_b"; do
    ...
    if [ -n "$meta" ]; then
      ...
      meta_age=$(echo "$meta" | cut -d'|' -f3)
      if [ "$meta_age" -gt 168 ]; then
        printf "%s[WARN]%s %s is older than 7 days (%s, %sh). Consider regenerating with option 9.\n" "$C_YLW" "$C_RESET" "$f" "$meta_date" "$meta_age"
      fi
    fi
  done
  ...
  FILE_A="$file_a" run_with_spinner "MIRROR" "Indexing A..." bash -c 'awk -F"\t" "{map[\$2]=\$1} END {for (p in map) print p\"\t\"map[p]}" "$FILE_A" | sort >"'"$STATE_DIR/mirror_a.tmp"'"'
  FILE_B="$file_b" run_with_spinner "MIRROR" "Indexing B..." bash -c 'awk -F"\t" "{map[\$2]=\$1} END {for (p in map) print p\"\t\"map[p]}" "$FILE_B" | sort >"'"$STATE_DIR/mirror_b.tmp"'"'
  run_with_spinner "MIRROR" "Missing in B..." bash -c 'join -v1 -t$'"'"'\t'"'"' "'"$STATE_DIR/mirror_a.tmp"'" "'"$STATE_DIR/mirror_b.tmp"'" >"'"$missing_in_b"'"'
  run_with_spinner "MIRROR" "Missing in A..." bash -c 'join -v2 -t$'"'"'\t'"'"' "'"$STATE_DIR/mirror_a.tmp"'" "'"$STATE_DIR/mirror_b.tmp"'" >"'"$missing_in_a"'"'
  run_with_spinner "MIRROR" "Hash mismatches..." bash -c 'join -t$'"'"'\t'"'"' "'"$STATE_DIR/mirror_a.tmp"'" "'"$STATE_DIR/mirror_b.tmp"'" | awk -F"\t" "{if (\$2!=\$3) print \$1\"\tA:\"\$2\"\tB:\"\$3}" >"'"$mismatch"'"'
...
action_state_health() {
  print_header
  refresh_artifact_state
  printf "%s[INFO]%s Super Doctor for _DJProducerTools\n" "$C_CYN" "$C_RESET"
  local free_base free_state report_count plans_count quar_count log_count
  free_base=$(df -h "$BASE_PATH" 2>/dev/null | awk 'NR==2{print $4" free, use "$5}')
  free_state=$(df -h "$STATE_DIR" 2>/dev/null | awk 'NR==2{print $4" free, use "$5}')
  ...
  for tool in ffprobe ffmpeg sox flac metaflac id3v2 mid3v2 shntool jq python3 shasum; do
    if ! command -v "$tool" >/dev/null 2>&1; then
      ...
      tools_versions+=("$tool => ${ver:-n/a}")
    fi
  done
  venv_status="No ML venv"
  if [ -f "$VENV_DIR/bin/activate" ]; then
    venv_status="Present"
...
PY
  ...
  fi
  {
    ...
    echo "BASE_PATH space: ${free_base:-n/a}"
    echo "STATE_DIR space: ${free_state:-n/a}"
    echo "STATE_DIR size:"
    ...
    echo "  quarantine/: ${ART_QUAR_COUNT:-0} files${ART_QUAR_SIZE:+, ${ART_QUAR_SIZE}}"
    echo "  reports/: ${ART_REPORTS_SIZE:-n/a}"
    echo
    echo "Counts:"
    ...
    if [ "${#tools_missing[@]}" -gt 0 ]; then
      echo "Missing tools:"
      printf "  - %s\n" "${tools_missing[@]}"
      echo "Suggested (Homebrew): brew install ffmpeg sox flac id3v2 shntool jq"
      echo "Mutagen (tags): pip install mutagen"
    else
    ...
    echo
    echo "Recommendations:"
    echo "  - If hash/snapshot/dupes plan are missing: run 9/10/27."
    echo "  - Review quarantine with 11/12; free space if large."
    echo "  - Use 57 (exclusions) before heavy scans."
  } >"$STATE_HEALTH_REPORT"
  printf "%s[OK]%s Doctor report: %s\n" "$C_GRN" "$C_RESET" "$STATE_HEALTH_REPORT"
  if [ "${#tools_missing[@]}" -gt 0 ]; then
...
action_ml_profile() {
  print_header
  printf "%s[INFO]%s Local AI profile: %s\n" "$C_CYN" "$C_RESET" "${ML_PROFILE:-LIGHT}"
  ...
  case "$prof" in
    1)
      ML_PROFILE="LIGHT"
      save_conf
      printf "%s[OK]%s Local AI profile: LIGHT\n" "$C_GRN" "$C_RESET"
      ;;
    2)
      ML_PROFILE="TF_ADV"
      save_conf
      printf "%s[OK]%s Local AI profile: TF_ADV (%s)\n" "$C_GRN" "$C_RESET" "$ML_PKGS_TF"
      ;;
    *)
      printf "%s[INFO]%s No changes.\n" "$C_CYN" "$C_RESET"
      ;;
  esac
  pause_enter
}
...
PY
  ...
  finish_status_line
  rm -f "$list_tmp"
  printf "%s[OK]%s LUFS plan generated: %s\n" "$C_GRN" "$C_RESET" "$out"
  pause_enter
}

action_audio_cues_onsets() {
  print_header
  printf "%s[INFO]%s Auto-cues by onsets (librosa; TSV plan).\n" "$C_CYN" "$C_RESET"
...
PY
  ...
  finish_status_line
  rm -f "$list_tmp"
  printf "%s[OK]%s Auto-cues generated: %s\n" "$C_GRN" "$C_RESET" "$out"
  pause_enter
}
  ...
  while true; do
    ...
    case "$pop" in
      ...
      2)
        ...
        if [ "${#plist[@]}" -eq 0 ]; then
          printf "%s[WARN]%s No profiles saved yet.\n" "$C_YLW" "$C_RESET"
          pause_enter
          continue
        fi
        ...
        printf "Profile to load (name or number, ENTER to cancel): "
        read -r pname
        ...
        for warn_path in "$BASE_PATH" "${AUDIO_ROOT:-}" "${GENERAL_ROOT:-}" "${SERATO_ROOT:-}" "${ABLETON_ROOT:-}"; do
          if [ -n "$warn_path" ] && [ ! -d "$warn_path" ]; then
            printf "%s[WARN]%s Path does not exist: %s\n" "$C_YLW" "$C_RESET" "$warn_path"
          fi
        done
        pause_enter
        ;;
...
chain_23_autopilot_quick() {
  chain_run_header "Auto-pilot: chains 5,16,21"
  chain_5_show_prep
  chain_16_clean_backup
  chain_21_multidisk_dedup
  printf "%s[OK]%s Auto-pilot quick run completed.\n" "$C_GRN" "$C_RESET"
  pause_enter
}
  ...
  while true; do
    ...
    case "$lop" in
      ...
      6)
        ...
        status_line "INVENTORY" "--" "Searching libraries..."
        {
...
action_H_help_info() {
  ...
  printf "  70: Local AI profile (LIGHT recommended; TF_ADV optional for Apple Silicon).\n\n"
