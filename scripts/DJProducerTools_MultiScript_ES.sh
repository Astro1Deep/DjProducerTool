...
# Asegura que el script se ejecuta desde la carpeta raíz del proyecto.
if [ ! -f "DJProducerTools_MultiScript_ES.sh" ] || [ ! -d "tests" ]; then
    printf "\n%s[ERROR] El script no se está ejecutando desde la carpeta raíz del proyecto.%s\n" "$C_RED" "$C_RESET" >&2
    printf "Por favor, navega a la carpeta 'DJProducerTools_Project' y ejecuta el script desde allí.\n" >&2
    exit 1
fi
...
DJ_SAFE_LOCK=1
DRYRUN_FORCE=0
DEBUG_MODE=0 # Cambiar a 1 para ver la salida detallada de los comandos
...
action_59_ml_profile() {
  # Permite al usuario elegir el perfil de IA (LIGHT o TF_ADV).
  print_header
...
status_line() {
  ...
  if [ "$DEBUG_MODE" -eq 1 ]; then
    # En modo debug, solo imprime el estado sin sobreescribir la línea.
...
invalid_option() {
  # Muestra un mensaje de opción inválida y pausa.
...
action_1_status() {
  print_header
  refresh_artifact_state
  # Muestra el estado actual: rutas, locks, y un resumen de los artefactos generados.
...
action_2_change_base() {
  # Permite al usuario cambiar el BASE_PATH principal del proyecto.
  print_header
...
action_3_summary() {
  # Muestra un resumen del uso de disco del BASE_PATH y los últimos reportes.
  print_header
...
action_4_top_dirs() {
  # Lista las carpetas de primer nivel por tamaño para identificar las más pesadas.
  print_header
...
action_5_top_files() {
  # Lista los archivos más grandes dentro del BASE_PATH.
  print_header
...
action_6_scan_workspace() {
  # Genera un listado completo de todos los archivos en el BASE_PATH.
  print_header
...
action_7_backup_serato() {
  # Realiza un backup específico de las carpetas _Serato_ y _Serato_Backup.
  print_header
...
action_8_backup_dj() {
  # Realiza un backup de metadatos de varias aplicaciones de DJ (Serato, Traktor, etc.).
  print_header
...
action_9_hash_index() {
  # Genera un índice con el hash SHA-256 de cada archivo para la detección de duplicados.
  print_header
...
action_10_dupes_plan() {
  # A partir del hash_index, crea un plan para gestionar duplicados exactos.
  print_header
...
action_unique_from_hash() {
  # Genera una lista de archivos únicos a partir de un hash_index.
  print_header
  hash_file="$REPORTS_DIR/hash_index.tsv"
  if [ ! -f "$hash_file" ]; then
    printf "%s[WARN]%s No hay hash_index.tsv, generando primero.\n" "$C_YLW" "$C_RESET"
...
action_11_quarantine_from_plan() {
  # Mueve los archivos marcados como "QUARANTINE" en el plan de duplicados a la carpeta de cuarentena.
  print_header
  plan_tsv="$PLANS_DIR/dupes_plan.tsv"
  if [ ! -f "$plan_tsv" ]; then
  ...
  if ! ensure_tool_installed "bc" "brew install bc"; then
    printf "%s[WARN]%s Se necesita 'bc' para calcular el espacio en disco. Se omitirá la comprobación de espacio.\n" "$C_YLW" "$C_RESET"
    pause_enter
  fi
...
action_12_quarantine_manager() {
  # Ofrece un menú para gestionar la cuarentena: listar, restaurar o purgar archivos.
  while true; do
    clear
...
action_13_ffprobe_report() {
  # Usa ffprobe para detectar archivos de media potencialmente corruptos.
...
action_14_playlists_per_folder() {
  # Crea un archivo de playlist .m3u8 en cada subdirectorio.
  print_header
...
action_15_relink_helper() {
  # Genera un reporte para ayudar a relinkear archivos movidos, incluyendo hashes opcionales.
  print_header
...
action_17_find_dj_libs() {
  # Busca y lista las rutas de las librerías de software de DJ conocidas.
  print_header
...
action_18_rescan_intelligent() {
  # Realiza un rescan "inteligente" del workspace (placeholder).
  print_header
...
action_19_tools_diag() {
  # Diagnostica la presencia de herramientas externas necesarias (ffprobe, rsync, etc.).
  print_header
...
action_21_install_cmd() {
  # Instala un comando de acceso rápido (symlink) en /usr/local/bin.
  print_header
...
action_22_uninstall_cmd() {
  # Desinstala el comando de acceso rápido.
  print_header
  target="/usr/local/bin/djproducertool"
...
action_23_toggle_safe_mode() {
  # Activa o desactiva el SAFE_MODE, que previene operaciones destructivas.
  print_header
...
action_24_toggle_lock() {
  # Activa o desactiva el DJ_SAFE_LOCK, una capa extra de seguridad.
  print_header
...
action_25_quick_help() {
  # Muestra una guía rápida con los flujos de trabajo más comunes.
  print_header
...
action_26_export_import_state() {
  # Exporta el estado completo de _DJProducerTools a un archivo .tar.gz.
  print_header
...
action_27_snapshot() {
  # Genera un "snapshot" rápido de integridad usando hashes.
  print_header
...
action_28_logs_viewer() {
  # Muestra los últimos archivos de log generados.
  print_header
...
action_29_toggle_dryrun() {
  # Activa o desactiva el modo DRYRUN_FORCE, que simula las acciones.
  print_header
...
action_60_reset_state() {
  # Permite resetear el estado de la aplicación (borrar _DJProducerTools).
  print_header
...
action_65_compare_hash_indexes() {
  # Compara dos archivos hash_index.tsv para encontrar diferencias.
  print_header
...
action_68_mirror_integrity_check() {
  # Valida la integridad de un mirror (espejo) comparando hashes entre dos rutas.
  print_header
...
action_66_state_health() {
  # Realiza un "Super Doctor", un chequeo integral del estado de la aplicación y sus artefactos.
  print_header
  refresh_artifact_state
...
action_67_export_import_config() {
  # Exporta o importa únicamente la configuración y los perfiles.
  print_header
...
action_56_toggle_ml() {
  # Activa o desactiva globalmente el uso del entorno de Machine Learning.
  print_header
  if [ "${ML_ENV_DISABLED:-0}" -eq 1 ]; then
    ML_ENV_DISABLED=0
    save_conf
...
action_55_ml_evo_manager() {
  # Gestiona el ML Evolutivo: entrenar, predecir o resetear el modelo local.
  print_header
...
action_57_tensorflow_manager() {
  # Gestiona la instalación opcional de TensorFlow para funciones avanzadas.
  print_header
  if [ "${ML_ENV_DISABLED:-0}" -eq 1 ]; then
...
action_58_tensorflow_lab() {
  # Submenú con herramientas experimentales que usan TensorFlow (auto-tagging, similitud).
  while true; do
    clear
    print_header
...
action_69_audio_lufs_plan() {
  # Analiza el loudness (LUFS) de los archivos de audio y crea un plan de normalización.
  print_header
...
action_70_audio_cues_onsets() {
  # Detecta onsets (transitorios) en el audio para sugerir puntos de CUE automáticos.
  print_header
...
action_31_report_tags() {
  # Genera un reporte de los tags de audio encontrados.
  print_header
...
action_32_serato_video_report() {
  # Genera un reporte de los archivos de video compatibles con Serato Video.
  print_header
...
action_33_serato_video_prep() {
  # Crea un plan para transcodificar videos para Serato Video.
  print_header
...
action_34_normalize_names() {
  # Crea un plan para normalizar nombres de archivo (eliminar caracteres extraños, etc.).
  print_header
...
action_35_samples_by_type() {
  # Crea un plan para organizar samples por tipo (kick, snare, etc.) basado en el nombre.
  print_header
...
action_37_web_whitelist_manager() {
  # Gestiona la whitelist de dominios para la limpieza de tags (placeholder).
  print_header
...
action_42_smart_analysis() {
  # (Deep-Thinking) Realiza un análisis inteligente de la librería y genera un reporte JSON.
  maybe_activate_ml_env "Opción 42 (Smart Analysis)"
  print_header
  printf "%s[INFO]%s 🧠 DEEP-THINKING: Análisis Inteligente (Opción 42)\n" "$C_CYN" "$C_RESET"
...
action_43_ml_predictor() {
  # (Machine Learning) Predice problemas potenciales en la librería basado en heurísticas.
  maybe_activate_ml_env "Opción 43 (Predictor ML)"
  print_header
  printf "%s[INFO]%s 🤖 MACHINE LEARNING: Predictor de Problemas (Opción 43)\n" "$C_CYN" "$C_RESET"
...
action_44_efficiency_optimizer() {
  # (Deep-Thinking) Genera un plan para optimizar la eficiencia de la librería.
  maybe_activate_ml_env "Opción 44 (Optimizador)"
  print_header
  printf "%s[INFO]%s ⚡ DEEP-THINKING: Optimizador de Eficiencia (Opción 44)\n" "$C_CYN" "$C_RESET"
...
action_45_smart_workflow() {
  # (Deep-Thinking) Propone un flujo de trabajo inteligente y optimizado.
  maybe_activate_ml_env "Opción 45 (Flujo inteligente)"
  print_header
  printf "%s[INFO]%s 🚀 DEEP-THINKING: Flujo de Trabajo Inteligente (Opción 45)\n" "$C_CYN" "$C_RESET"
...
action_46_integrated_dedup() {
  # (Deep-Thinking) Ofrece una visión integrada de la deduplicación (exacta y fuzzy).
  maybe_activate_ml_env "Opción 46 (Deduplicación integrada)"
  print_header
  printf "%s[INFO]%s 🔄 DEEP-THINKING: Deduplicación Integrada (Opción 46)\n" "$C_CYN" "$C_RESET"
...
action_47_ml_organization() {
  # (Machine Learning) Sugiere una organización automática de archivos basada en el nombre.
  maybe_activate_ml_env "Opción 47 (Organización ML)"
  print_header
  printf "%s[INFO]%s 📂 MACHINE LEARNING: Organización Automática (Opción 47)\n" "$C_CYN" "$C_RESET"
...
action_48_metadata_harmonizer() {
  # (Deep-Thinking) Crea un plan para armonizar y estandarizar metadatos.
  maybe_activate_ml_env "Opción 48 (Armonizador metadatos)"
  print_header
  printf "%s[INFO]%s 🎵 DEEP-THINKING: Armonizador de Metadatos (Opción 48)\n" "$C_CYN" "$C_RESET"
...
action_49_predictive_backup() {
  # (Machine Learning) Propone una estrategia de backup predictiva basada en el análisis de riesgo.
  maybe_activate_ml_env "Opción 49 (Backup predictivo)"
  print_header
  printf "%s[INFO]%s 🛡️ MACHINE LEARNING: Backup Predictivo (Opción 49)\n" "$C_CYN" "$C_RESET"
...
action_50_cross_platform_sync() {
  # (Deep-Thinking) Genera un plan para la sincronización entre diferentes plataformas de DJ.
  maybe_activate_ml_env "Opción 50 (Sync multiplataforma)"
  print_header
  printf "%s[INFO]%s 🌐 DEEP-THINKING: Sincronización Multi-Plataforma (Opción 50)\n" "$C_CYN" "$C_RESET"
...
action_51_advanced_analysis() {
  # (Deep-Thinking) Realiza un análisis avanzado y profundo de la librería.
  maybe_activate_ml_env "Opción 51 (Análisis avanzado)"
  print_header
  printf "%s[INFO]%s 🔬 DEEP-THINKING: Análisis Avanzado Profundo (Opción 51)\n" "$C_CYN" "$C_RESET"
...
action_52_integration_engine() {
  # (Deep-Thinking) Propone un motor de integración para combinar varias acciones.
  maybe_activate_ml_env "Opción 52 (Motor integración)"
  print_header
  printf "%s[INFO]%s ⚙️ DEEP-THINKING: Motor de Integración (Opción 52)\n" "$C_CYN" "$C_RESET"
...
action_53_adaptive_recommendations() {
  # (Machine Learning) Ofrece recomendaciones adaptativas y priorizadas.
  maybe_activate_ml_env "Opción 53 (Recomendaciones adaptativas)"
  print_header
  printf "%s[INFO]%s 💡 MACHINE LEARNING: Recomendaciones Adaptativas (Opción 53)\n" "$C_CYN" "$C_RESET"
...
action_54_automated_cleanup_pipeline() {
  # (Deep-Thinking) Define un pipeline completo y automatizado de limpieza.
  maybe_activate_ml_env "Opción 54 (Pipeline automático)"
  print_header
  printf "%s[INFO]%s 🔄 DEEP-THINKING: Pipeline de Limpieza Automatizado (Opción 54)\n" "$C_CYN" "$C_RESET"
...
chain_13_hash_mirror_check() {
  chain_run_header "Hash + mirror check (9 -> 68)"
  if ensure_tool_installed "shasum" "brew install coreutils"; then
    action_9_hash_index
    action_68_mirror_integrity_check
  fi
...
chain_14_audio_prep() {
  chain_run_header "Audio prep (31 -> 69 -> 70)"
  if ensure_tool_installed "python3"; then
    if ensure_python_package_installed "mutagen"; then
        action_31_report_tags
    fi
...
chain_15_integrity_audit() {
  chain_run_header "Auditoría completa de integridad (6 -> 9 -> 27 -> 68)"
  action_6_scan_workspace
  if ensure_tool_installed "shasum" "brew install coreutils"; then
    action_9_hash_index
    action_27_snapshot
...
chain_21_multidisk_dedup() {
  chain_run_header "Dedup multi-disco + espejo (9 -> 10 -> 46 -> 11 -> 68)"
  if ensure_tool_installed "shasum" "brew install coreutils"; then
    action_9_hash_index
    action_10_dupes_plan
    action_46_integrated_dedup
...
action_69_artist_pages() {
  # Gestiona una plantilla con los perfiles y links de un artista para presskits.
  print_header
...
action_40_wav_to_mp3() {
  # Convierte archivos WAV a MP3 de alta calidad (320kbps).
  print_header
...
action_41_update_self() {
  # Busca y aplica actualizaciones del script desde GitHub.
  print_header
...
action_L8_metadata_consistency_report() {
  # Reporta inconsistencias entre los tags ID3 (Artista/Título) y el nombre del archivo.
  print_header
...
action_L9_low_bitrate_report() {
  # Reporta archivos de audio con un bitrate inferior a 320kbps.
  print_header
...
action_L10_no_tags_report() {
  # Reporta archivos de audio que no tienen tags ID3 de Artista o Título.
  print_header
...
action_71_chains() {
  # Muestra el submenú de cadenas automatizadas.
  while true; do
    clear
    print_header
    ...
    printf "%s9)%s Backup predictivo (49 -> 8 -> 27)\n" "$C_YLW" "$C_RESET"
    printf "%s10)%s Sync multiplataforma (50 -> 39 -> 8 -> 8)\n" "$C_YLW" "$C_RESET"
    printf "%s11)%s Diagnóstico rápido (1 -> 3 -> 4 -> 5)\n" "$C_YLW" "$C_RESET"
    printf "%s12)%s Salud Serato (7 -> 59)\n" "$C_YLW" "$C_RESET"
    printf "%s13)%s Hash + mirror check (9 -> 68)\n" "$C_YLW" "$C_RESET"
    printf "%s14)%s Audio prep (31 -> 69 -> 70)\n" "$C_YLW" "$C_RESET"
    printf "%s15)%s Auditoría integridad (6 -> 9 -> 27 -> 68)\n" "$C_YLW" "$C_RESET"
    printf "%s16)%s Limpieza + backup seguro (39 -> 34 -> 10 -> 11 -> 8 -> 27)\n" "$C_YLW" "$C_RESET"
    printf "%s17)%s Prep sync librerías (18 -> 14 -> 50 -> 8 -> 27)\n" "$C_YLW" "$C_RESET"
    printf "%s18)%s Salud video/visuales (V2 -> V6 -> V8 -> V9 -> 8)\n" "$C_YLW" "$C_RESET"
    printf "%s19)%s Organización audio avanzada (31 -> 30 -> 35 -> 45 -> 46)\n" "$C_YLW" "$C_RESET"
    printf "%s20)%s Seguridad Serato reforzada (7 -> 8 -> 59 -> 12 -> 49)\n" "$C_YLW" "$C_RESET"
    printf "%s21)%s Dedup multi-disco + espejo (9 -> 10 -> 46 -> 11 -> 68)\n" "$C_YLW" "$C_RESET"
    printf "%s22)%s Pack presskit (72 -> export)\n" "$C_YLW" "$C_RESET"
...
action_V1_ableton_report() {
  # Genera un reporte de los sets de Ableton (.als), listando samples y plugins.
  print_header
...
action_V2_visuals_inventory() {
  # Crea un inventario de todos los archivos de video/visuales.
  print_header
...
action_V3_osc_dmx_plan() {
  # Crea un plan de ejemplo para controlar luces (DMX) o video (OSC).
  print_header
...
action_V4_serato_video_report() {
  # Alias para action_32_serato_video_report.
  action_32_serato_video_report
}

action_V5_serato_video_prep() {
  # Alias para action_33_serato_video_prep.
  action_33_serato_video_prep
}

action_V6_visuals_ffprobe_report() {
  # Genera un reporte detallado de los archivos de video usando ffprobe (resolución, codec, etc.).
  print_header
...
action_V7_visuals_by_resolution() {
  # Crea un plan para organizar los videos en carpetas según su resolución.
  print_header
...
action_V8_visuals_hash_dupes() {
  # Detecta duplicados exactos de archivos de video usando hashes.
  print_header
...
action_V9_visuals_optimize_plan() {
  # Crea un plan con sugerencias para optimizar videos (ej. transcodificar a H.264).
  print_header
...
action_V10_osc_from_playlist() {
  # Genera un plan de cues OSC a partir de una playlist.
  print_header
...
action_V11_dmx_from_playlist() {
  # Genera un plan de escenas DMX a partir de una playlist.
  print_header
  default_pl="$BASE_PATH/playlist.m3u8"
...
action_V12_dmx_presets() {
  # Muestra una plantilla de presets DMX para equipos comunes (Spider, Láser).
  print_header
...
action_V13_dmx_fixtures_inventory() {
  # Busca archivos de definición de fixtures DMX.
  print_header
...
action_V14_visuals_transcode_adv() {
  # Crea un plan de transcodificación de video avanzado con comandos ffmpeg.
  print_header
...
main_loop() {
  while true; do
    ...
    case "$op" in
      ...
      42) action_42_smart_analysis ;;
      43) action_43_ml_predictor ;;
      44) action_44_efficiency_optimizer ;;
      45) action_45_smart_workflow ;;
      46) action_46_integrated_dedup ;;
      47) action_47_ml_organization ;;
      48) action_48_metadata_harmonizer ;;
      49) action_49_predictive_backup ;;
      50) action_50_cross_platform_sync ;;
      51) action_51_advanced_analysis ;;
      52) action_52_integration_engine ;;
      53) action_53_adaptive_recommendations ;;
      54) action_54_automated_cleanup_pipeline ;;
      55) action_55_ml_evo_manager ;;
      56) action_56_toggle_ml ;;
      57) action_57_tensorflow_manager ;;
      58) action_58_tensorflow_lab ;;
      59) action_59_ml_profile ;;
      60) action_60_reset_state ;;
      61) submenu_profiles_manager ;;
      62) submenu_ableton_tools ;;
      63) submenu_importers_cues ;;
      64) submenu_excludes_manager ;;
      65) action_65_compare_hash_indexes ;;
      66) action_66_state_health ;;
      67) action_67_export_import_config ;;
      68) action_68_mirror_integrity_check ;;
      69) action_69_audio_lufs_plan ;;
      70) action_70_audio_cues_onsets ;;
      71) action_71_chains ;;
      72) action_72_artist_pages ;;
      A|a) action_71_chains ;;
